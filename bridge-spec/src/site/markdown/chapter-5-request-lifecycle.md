...
![Faces Lifecyle Diagram](images/faces-lifecycle-diagram.png)
...
![Portlet Lifecycle Comparison](images/portlet-1-and-2-lifecycle-comparison.png)
...
![Faces Action Lifecycle in Bridge](images/faces-action-lifecycle-in-bridge.png)

![Faces Render Lifecycle in Bridge](images/faces-render-lifecycle-in-bridge.png)
...
[5.2.5](chapter-5-request-lifecycle.html#5.2.5).
...
section [5.2.7](chapter-5-request-lifecycle.html#5.2.7).
...
subsequent render requests that are identified as pertaining to that action (or event). The term used to describe the
scope of this managed data is the **bridge request scope**. The bridge request scope manages two types of Faces data:
the Faces view state and the additional Faces request scope data Faces relies on when executing both the action and the
render in a single request. Though the Faces view state management is commonly delegated to the normal Faces
...
- The Faces request scope attributes returned by calling `ExternalContext.getRequestMap()` except for:
    ...
    - those explicitly excluded by mechanisms described in [5.1.2.1](chapter-5-request-lifecycle.html#5.1.2.1)
...
the start of a new scope so never restore a preexisting one<sup>[[5.2](tck-tests.html#5.2)]</sup>. **Note**: if the
...
scope if the action doesn't terminate because of a redirect<sup>[[5.3](tck-tests.html#5.3)]</sup> or the navigational
target of the action doesn't specify a portlet mode that differs from the current
request<sup>[[5.4](tck-tests.html#5.4)]</sup>. Such state may either be managed directly in a scoped data structure or
...
scope<sup>[[5.48](tck-tests.html#5.48)]</sup>, otherwise create a new scope.
...
redirect<sup>[[5.49](tck-tests.html#5.49)]</sup> or the navigational target of the event specifies a portlet mode that
differs from the current request<sup>[[5.50(tck-tests.html#5.50)]</sup>. Such state may either be managed directly in a
...
- For each render request, if a bridge request scope exists, restore its state to the current (container) request
scope<sup>[[5.5](tck-tests.html#5.5)]</sup>. For any action parameters that have been preserved, the parameter is
...
after the lifecycle has run<sup>[[5.51](tck-tests.html#5.51)]</sup>. For any action parameters that have been preserved,
...
it exists or otherwise a newly created bridge request scope<sup>[[5.52](tck-tests.html#5.52)]</sup>. Such state may
...
subsequent bridge request scope is established)<sup>[[5.8](tck-tests.html#5.8)]</sup>. The exception is when the bridge
...
action) are executed in the same bridge request scope<sup>[[5.9](tck-tests.html#5.9)]</sup>. There are a variety of
reasons that subsequent render requests might reference differing request scopes including a mode change, processing a
...
[5.1.2](chapter-5-request-lifecycle.html#5.1.2), the extended bridge request scope is inclusive. It includes application
...
reachable by this web application<sup>[[5.10b](tck-tests.html#5.10)]</sup>.
...
[portlet2.0-bridge-faces1.2-faces-config-extensions.xsd](xsd/portlet2.0-bridge-faces1.2-faces-config-extensions.xsd)
when such element structure appears as a child of the `<application-extension>` element in any faces-config.xml
reachable by this web application<sup>[[5.10c](tck-tests.html#5.10)]</sup>.
...
that portlet is the target of the current request<sup>[[5.10e](tck-tests.html#5.10)]</sup>.
...
bridge must acquire the FacesContext by calling the `FacesContextFactory`<sup>[[5.12](tck-tests.html#5.12)]</sup>. The
...
[5.2.1](chapter-5-request-lifecycle.html#5.2.1)<sup>[[5.13](tck-tests.html#5.13)]</sup>. For example:
...
request object<sup>[[5.14](tck-tests.html#5.14)]</sup>:
...
`doFacesRequest()` is passed an `EventRequest` and `EventResponse`. The value `RENDER_PHASE` must be set when
...
viewId<sup>[[5.15](tck-tests.html#5.15)]</sup>.
- If the request attribute named `javax.portlet.faces.viewPath` is non-null then use this value to process the
`ContextPath` relative path and extract the target `viewId`<sup>[[5.16](tck-tests.html#5.16)]</sup>. If unable to
extract a `viewId` from the path throw
`javax.portlet.faces.BridgeInvalidViewPathException`<sup>[[5.17](tck-tests.html#5.17)]</sup>.
- If the bridge is processing for the target view in a render request which occurs in the same portlet mode following
another render request in which a redirect occurred and an action, event, or resource request hasn't been processed in
the meantime, use the target `viewId` from the prior redirect<sup>[[5.18](tck-tests.html#5.18)]</sup>.
...
into the response<sup>[[5.19](tck-tests.html#5.19)]</sup>. Details of such encoding/decoding are implementation
specific.
...
parameters in this query string as additional request parameters<sup>[[5.21](tck-tests.html#5.21)]</sup>.
...
either the bridge's construction or a prior `release()`)<sup>[[5.22](tck-tests.html#5.22)]</sup>.
...
considered complete<sup>[[5.24](tck-tests.html#5.24)]</sup>:
    ...
    [5.3.3](chapter-5-request-lifecycle.html#5.3.3).<sup>[[5.74](tck-tests.html#5.74)]</sup>
    - if the current view (of the action navigation) is a Faces view which doesn't indicate a need to switch from the
    current portlet mode then satisfy all requirements listed in section [5.1.2](chapter-5-request-lifecycle.html#5.1.2)
...
- remove the `javax.portlet.faces.phase` request attribute<sup>[[5.26](tck-tests.html#5.26)]</sup>.
...
- throw the BridgeUninitializedException if the bridge isn't currently initialized (`init()` has been called following
either the bridge's construction or a prior `release()`)<sup>[[5.53](tck-tests.html#5.53)]</sup>.
...
request<sup>[[5.55](tck-tests.html#5.55)]</sup>.
- reestablish (if so indicated) the Faces request scope from the corresponding bridge request scope satisfying all
requirements listed in section [5.1.2](chapter-5-request-lifecycle.html#5.1.2) concerning providing an idempotent
rendition based on the `viewId` and request scope state referenced in the render
request<sup>[[5.48](tck-tests.html#5.48)]</sup>.
- acquire the `FacesContext` and `Lifecycle` as described above ensuring the appropriate target `viewId` will be
processed [5.2.3](chapter-5-request-lifecycle.html#5.2.3). **Note**: if the event request follows an action request, the
...
`execute` method on the `Lifecycle` object acquired in section [5.2.1](chapter-5-request-lifecycle.html#5.2.1). For
example:
...
retaining the returned `EventNavigationResult`<sup>[[5.56](tck-tests.html#5.56)]</sup>.
...
handling):
    ...
    - set new public render parameters from mapped models whose values have changed as per section
    [5.3.3](chapter-5-request-lifecycle.html#5.3.3) <sup>[[5.58](tck-tests.html#5.58)]</sup>.
    - if the current view (of the action navigation) is a Faces view which doesn't indicate a need to switch from the
    current portlet mode then satisfy all requirements listed in section [5.1.2](chapter-5-request-lifecycle.html#5.1.2)
    concerning establishing and maintaining a bridge request scope with its corresponding state.
- release the FacesContext<sup>[[5.59](tck-tests.html#5.59)]</sup>.
- remove the javax.portlet.faces.phase request attribute<sup>[[5.60](tck-tests.html#5.60)]</sup>.
...
and then returns<sup>[[5.61](tck-tests.html#5.61)]</sup>.
...
- throw the BridgeUninitializedException if the bridge isn't currently initialized (`init()` has been called following
either the bridge's construction or a prior `release()`)<sup>[[5.27](tck-tests.html#5.27)]</sup>.
- set the `javax.portlet.faces.phase` request attribute to `Bridge.PortletPhase.RENDER_PHASE` prior to acquiring the
...
requirements listed in section [5.1.2](chapter-5-request-lifecycle.html#5.1.2) concerning providing an idempotent
rendition based on the `viewId` and request scope state referenced in the render
request<sup>[[5.30](tck-tests.html#5.30)]</sup>.
- ensure the `RenderKits ResponseStateManager isPostback()` method returns true if and only if a bridge request scope
has been identified and restored<sup>[[5.31](tck-tests.html#5.31)]</sup>. **Note**: as indicated in section
[5.1.2](chapter-5-request-lifecycle.html#5.1.2) this is controlled by both restoring the value of the
...
[5.2.1](chapter-5-request-lifecycle.html#5.2.1). For example:
...
Faces<sup>[[5.32](tck-tests.html#5.32)]</sup>.
- ensure that only the Faces `RestoreView` (if necessary) phases executes. I.e. the bridge must ensure that the Faces
action phases following the `RestoreView` phase are skipped<sup>[[5.33a](tck-tests.html#5.33)]</sup>.
...
- provide a `PhaseListener` that recognizes the `RESTORE_VIEW afterPhase`. Within this notification process any public
render parameters as per section [5.3.2](chapter-5-request-lifecycle.html#5.3.2)
- execute the `render` phase of the Faces lifecycle<sup>[[5.34](tck-tests.html#5.34)]</sup>. This is accomplished by
...
scope<sup>[[5.36](tck-tests.html#5.36)]</sup>.
- release the `FacesContext`<sup>[[5.37](tck-tests.html#5.37)]</sup>.
- remove the `javax.portlet.faces.phase request` attribute<sup>[[5.38](tck-tests.html#5.38)]</sup>.
...
- If the request targets a Faces resource:
    ...
    scope<sup>[[5.65](tck-tests.html#5.65)]</sup>.
    - ensure the corresponding Faces `view` is (re)established for the targeted Faces `viewId`. This is accomplished by
    calling the `execute` method on the `Lifecycle` object acquired in section
    [5.2.1](chapter-5-request-lifecycle.html#5.2.1). For example:
    ...
    `Lifecycle` object acquired in section [5.2.1](chapter-5-request-lifecycle.html#5.2.1). For example:

            lifecycle.render(context);

    - preserve any changes in the current request scope to the bridge request. Reuse the existing bridge request scope
    if it exists, otherwise create one<sup>[[5.66](tck-tests.html#5.66)]</sup>.
...
the Faces lifecycle has executed and before any other phase runs<sup>[[5.69](tck-tests.html#5.69)]</sup>. This allows
...
application-extension` section See [5.3.1](chapter-5-request-lifecycle.html#5.3.1), the bridge calls the set accessor of
the associated bean's mapped data member<sup>[[5.70](tck-tests.html#5.70)]</sup>. The accessor must accept a `String`
...
`FacesContext`<sup>[[5.71](tck-tests.html#5.71)]</sup>. This gives the portlet an opportunity to trigger needed model
updates or recomputations.

### <a name="5.3.3"></a>5.3.3  Processing outgoing Public Render Parameters (changes)

Before returning from processing a Faces action<sup>[[5.72](tck-tests.html#5.72)]</sup> or
event<sup>[[5.73](tck-tests.html#5.73)]</sup> request, the bridge reprocesses the public parameter mappings to determine
...
[5.3.1](chapter-5-request-lifecycle.html#5.3.1). For each mapped model in the `public-parameter-mappings` section of the
...
default `viewId` used instead<sup>[[5.39](tck-tests.html#5.39)]</sup>.
...
mode<sup>[[5.42](tck-tests.html#5.42)]</sup>:
...
one such attribute per supported portlet mode<sup>[[5.43](tck-tests.html#5.43)]</sup>. The value of this attribute is
the last `viewId` that was active for a given mode<sup>[[5.44](tck-tests.html#5.44)]</sup>. If a mode has never been
active then this attribute's value must be the same as the that mode's default
`viewId`<sup>[[5.45](tck-tests.html#5.45)]</sup>. Furthermore, if not encoding the default `viewId`, this `viewId` value
...
the history list<sup>[[5.46](tck-tests.html#5.46)]</sup>.
...
history list only be updated during the render phase. As Portlet 1.0 doesn't carry forward postback parameters into the
render phase one can infer the render parameters during the render phase as they should be the only parameters
received.

- the bridge must allow, recognize, and evaluate `faces-config.xml` navigation rules where the `<to-view-id>` element is
a JSF expression<sup>[[5.47](tck-tests.html#5.47)]</sup>.
