...
**header request:** a *portlet request* to generate markup for the portal's <head> section based on the current portlet
state.
...
**header response:** a portlet's reply to a header request. This is the <head> section markup response. As the portlet
model assumes many renders of the portlet's headers may occur after an action has been processed, rendering the
portlet's headers is expected to be idempotent. Portlets are discouraged from changing state during a header request.
Instead they merely render from their existing internal state and that supplied in the request from the consumer
(portal).
