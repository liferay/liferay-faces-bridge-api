...
[<a name="3.1"></a>3.1] For this specification, the returned name must be "Portlet 3.0 Bridge for JavaServer Faces 2.2"
...
[<a name="6.141"></a>6.141] getRequestServerName() (bridge-tck-main-portlet)
...
[<a name="6.142"></a>6.142] getRequestServerPort() (bridge-tck-main-portlet)
...
[<a name="6.143"></a>6.143] getRealPath(String) (bridge-tck-main-portlet)
...
- In the RESOURCE_PHASE, call `externalContext.setResponseHeader("foo", "6789")`

- In the `onreadystatechange` callback, call `getResponseHeader("foo")` and verify that the return value is equal to `"1234, 6789"`.

- Update the markup of the form to indicate the outcome of the test.

[<a name="6.150"></a>6.150] responseReset() (bridge-tck-main-portlet)
...
[<a name="6.152"></a>6.152] responseFlushBuffer() (bridge-tck-main-portlet)
...
- In the RESOURCE_PHASE, call `externalContext.responseFlushBuffer()`

- In the RESOURCE_PHASE, call `externalContext.setResponseHeader("bar", "6789")`

- In the `onreadystatechange` callback, call `getResponseHeader("foo")` and verify that the return value is equal to `"1234"` and that calling `getResponseHeader("bar")` returns a `null` value.

- Update the markup of the form to indicate the outcome of the test.

[<a name="6.153"></a>6.153] TCK compatibility for setResponseContentLength(int) is satisfied by the following JSF
Showcase use-case:
...
[<a name="6.157"></a>6.157] getSessionMaxInactiveInterval() (bridge-tck-main-portlet)
...
[<a name="6.158"></a>6.158] setSessionMaxInactiveInterval(int) (bridge-tck-main-portlet)
...
- During the `ACTION_PHASE` of the portlet lifecycle, set a value on a `@BridgeRequestScoped` bean acquired by
  `@Inject`. The value must be the result of calling the `getClass().getSimpleName()` method on the `ActionRequest`
  bean acquired by `@Inject`. During the `HEADER_PHASE` of the portlet lifecycle, get the value from the
  `@BridgeRequestScoped` bean acquired by `@Inject`. The value that was set must match the value that is retrieved. This
  proves that an alternative producer returned the TCK's decorated `ActionRequest` object.

[<a name="7.8"></a>7.8] actionResponseAlternativeTest (bridge-tck-cdi1-portlet)

- During the `ACTION_PHASE` of the portlet lifecycle, set a value on a `@BridgeRequestScoped` bean acquired by
  `@Inject`. The value must be the result of calling the `getClass().getSimpleName()` method on the `ActionResponse`
  bean acquired by `@Inject`. During the `HEADER_PHASE` of the portlet lifecycle, get the value from the
  `@BridgeRequestScoped` bean acquired by `@Inject`. The value that was set must match the value that is retrieved. This
  proves that an alternative producer returned the TCK's decorated `ActionResponse` object.

[<a name="7.9"></a>7.9] clientDataRequestAlternativeTest (bridge-tck-cdi1-portlet)

- During the `ACTION_PHASE` of the portlet lifecycle, set a value on a `@BridgeRequestScoped` bean acquired by
  `@Inject`. The value must be the result of calling the `getClass().getSimpleName()` method on the `ClientDataRequest`
  bean acquired by `@Inject`. During the `HEADER_PHASE` of the portlet lifecycle, get the value from the
  `@BridgeRequestScoped` bean acquired by `@Inject`. The value that was set must match the value that is retrieved. This
  proves that an alternative producer returned the TCK's decorated `ClientDataRequest` object.
...
- Specify a `<factory-extension>` for `<bridge:bridge-portlet-config-factory>` that wraps the
  ...
  `@ContextPath`. If the expected value is returned, then this proves that an alternative producer returned the TCK's
  decorated context path.
...
- During the `ACTION_PHASE` of the portlet lifecycle, publish a portlet event. During the `EVENT_PHASE` of the portlet
  ...
  `EventRequest` that it required via `@Inject`. During the `HEADER_PHASE` of the portlet lifecycle, get the value from
  the `@BridgeRequestScoped` bean acquired by `@Inject`. The value that was set must match the value that is retrieved.
  This proves that an alternative producer returned the TCK's decorated `EventRequest` object.
...
- Specify a `<factory-extension>` for `<bridge:bridge-portlet-request-factory>` that wraps the
  `BridgePortletRequestFactory` of the FacesBridge implementation with a decorated `HeaderRequest`. During the
  `HEADER_PHASE` of the portlet lifecycle, get the class FQCN of the `HeaderRequest` acquired by `@Inject`. If the
  expected FQCN is returned, then this proves that an alternative producer returned the TCK's decorated `HeaderRequest`
  object.
...
- Specify a `<factory-extension>` for `<bridge:bridge-portlet-response-factory>` that wraps the
  ...
  `HEADER_PHASE` of the portlet lifecycle, get the class FQCN of the `HeaderResponse` acquired by `@Inject`. If the
...
- Specify a `<factory-extension>` for `<bridge:bridge-portlet-config-factory>` that wraps the
  ...
  by `@Inject`. If the expected value is returned, then this proves that an alternative producer returned the TCK's
  decorated `PortletContext` object.

[<a name="7.22"></a>7.22] portletModeAlternativeTest (bridge-tck-cdi1-portlet)

- Specify a `<factory-extension>` for `<bridge:bridge-portlet-request-factory>` that wraps the
  `BridgePortletRequestFactory` of the FacesBridge implementation with a decorated `HeaderRequest`. During the
  `HEADER_PHASE` of the portlet lifecycle, get the class FQCN of the `PortletMode` acquired by `@Inject`. If the expected
  FQCN is returned, then this proves that an alternative producer returned the TCK's decorated `PortletMode` object.
...
- Specify a `<factory-extension>` for `<bridge:bridge-portlet-config-factory>` that wraps the
  `BridgePortletConfigFactory` of the FacesBridge implementation with a decorated `PortletConfig`. During the
  `HEADER_PHASE` of the portlet lifecycle, get the value of the `String` acquired by `@Inject` with qualifier
  `@PortletName`. If the expected value is returned, then this proves that an alternative producer returned the TCK's
  decorated portlet name.

[<a name="7.24"></a>7.24] portletPreferencesAlternativeTest (bridge-tck-cdi1-portlet)

- Specify a `<factory-extension>` for `<bridge:bridge-portlet-request-factory>` that wraps the
  `BridgePortletRequestFactory` of the FacesBridge implementation with a decorated `HeaderRequest`. During the
  `HEADER_PHASE` of the portlet lifecycle, get the class FQCN of the `PortletPreferences` acquired by `@Inject`. If the
  expected FQCN is returned, then this proves that an alternative producer returned the TCK's decorated
...
- Specify a `<factory-extension>` for `<bridge:bridge-portlet-request-factory>` that wraps the
  `BridgePortletRequestFactory` of the FacesBridge implementation with a decorated `HeaderRequest`. During the
  `HEADER_PHASE` of the portlet lifecycle, get the class FQCN of the `PortletSession` acquired by `@Inject`. If the
  expected FQCN is returned, then this proves that an alternative producer returned the TCK's decorated `PortletSession`
  object.
...
- Specify a `<factory-extension>` for `<bridge:bridge-portlet-request-factory>` that wraps the
  `BridgePortletRequestFactory` of the FacesBridge implementation with a decorated `HeaderRequest`. During the
  `HEADER_PHASE` of the portlet lifecycle, get the class FQCN of the `RenderParameters` acquired by `@Inject`. If the
  expected FQCN is returned, then this proves that an alternative producer returned the TCK's decorated
...
- Specify a `<factory-extension>` for `<bridge:bridge-portlet-request-factory>` that wraps the
  `BridgePortletRequestFactory` of the FacesBridge implementation with a decorated `HeaderRequest`. During the
  `HEADER_PHASE` of the portlet lifecycle, get the class FQCN of the `RenderRequest` acquired by `@Inject`. If the
  expected FQCN is returned, then this proves that an alternative producer returned the TCK's decorated `RenderRequest`
  object, which is actually a `HeaderRequest` object since the Faces lifecycle runs in the `HEADER_PHASE` rather than
...
- Specify a `<factory-extension>` for `<bridge:bridge-portlet-request-factory>` that wraps the
  `BridgePortletRequestFactory` of the FacesBridge implementation with a decorated `ResourceRequest`. During the
  `RESOURCE_PHASE` of the portlet lifecycle, get the class FQCN of the `ResourceRequest` acquired by `@Inject`. If the
  expected FQCN is returned, then this proves that an alternative producer returned the TCK's decorated
...
- Specify a `<factory-extension>` for `<bridge:bridge-portlet-request-factory>` that wraps the
  `BridgePortletRequestFactory` of the FacesBridge implementation with a decorated `HeaderRequest`. During the
  `HEADER_PHASE` of the portlet lifecycle, get the value of the `String` acquired by `@Inject` with qualifier
  `@WindowId`. If the expected value is returned, then this proves that an alternative producer returned the TCK's
  decorated window id.
